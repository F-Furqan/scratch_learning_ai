...
import type { InstructorProfile, Paginated, SeoPayload } from '@/types';

defineProps<{
    instructors: Paginated<InstructorProfile>;
    seo: SeoPayload;
}>();
...
<template>
    ...
    <section class="relative bg-[#101827] text-white">
        ...
        />
        <div
            class="absolute inset-0 bg-[linear-gradient(90deg,rgba(16,24,39,0.98),rgba(16,24,39,0.82),rgba(16,24,39,0.5))]"
        />
        <div class="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
            <div class="max-w-3xl">
                ...
                >
                    Instructors with verified expertise.
                </h1>
                <p class="mt-5 max-w-2xl text-lg leading-8 text-slate-200">
                    Approved instructors with course expertise, editorial
                    history, and expert author signals.
                </p>
            </div>
        </div>
    </section>
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            >
                <Link
                    ...
                    :key="instructor.id"
                    :href="instructor.url"
                    class="group grid gap-5 rounded-md border border-slate-200 bg-white p-6 shadow-[0_16px_45px_rgba(15,23,42,0.08)] transition duration-300 hover:-translate-y-1 hover:border-teal-200 hover:shadow-[0_22px_60px_rgba(15,118,110,0.14)]"
                >
                    <div class="flex items-center gap-4">
                        <img
                            :src="
                                ...
                                fallbackAvatars[
                                    instructor.id % fallbackAvatars.length
                                ]
                            "
                    ...
                    <div
                        v-if="instructor.badges.length"
                        class="flex flex-wrap gap-2"
                    >
                        <span
                            v-for="badge in instructor.badges"
                            :key="badge.id"
                            class="rounded-md bg-teal-50 px-2 py-1 text-xs font-black text-teal-800"
                        >
                            {{ badge.name }}
                        </span>
                    </div>
            ...
            >
                Approved instructor profiles will appear here.
            </div>

            <div class="mt-8">
                <PublicPagination :links="instructors.links" />
            </div>
        </div>
    </section>
</template>
